...
public class ExecuteIB_ML20M_1 {
	public static void main(String[] args)
	{
		Object[] predictors = {new NonPersonalisedPredictor(), new SimpleAveragePredictor(), new WeightedAveragePredictor(), 
				new DeviationFromItemMeanPredictor()};
		String[] p_names = {"NonPersonalisedPredictor", "SimpleAveragePredictor", 
				"WeightedAveragePredictor", "DeviationFromItemMeanPredictor"};
		for (int i = 0; i< predictors.length; i++)
		{
			System.out.println(p_names[i] + "is running!!!");
			for (int k = 10; k <= 250; k += 10)
			{
				// configure the item-based CF algorithm - set the predictor, neighbourhood and similarity metric ...
				Predictor predictor = (Predictor) predictors[i];
